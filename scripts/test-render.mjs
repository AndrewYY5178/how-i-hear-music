...
const themeSource = await readFile(new URL('../modules/layout/theme.js', import.meta.url), 'utf8');
...
assert.match(mergedTaste, /class="philosophy-disclosure"/);
assert.match(mergedTaste, /LISTENING PRINCIPLES/);
assert.match(mergedTaste, /class="essay-item"/);
assert.doesNotMatch(mergedTaste, /class="essay-item-toggle"/);
assert.doesNotMatch(mergedTaste, /<details class="essay-item"/);
assert.match(stylesheet, /\.philosophy-disclosure>summary \{ display:flex;/);
...
assert.match(trackIndex, /class="track-card-link"/);
assert.doesNotMatch(trackIndex, />Open track<\/a>/);
assert.match(stylesheet, /\.track-card-link \{ position:absolute; z-index:1; inset:0;/);
...
assert.match(albumDetail, /data-album-wave-canvas/);
...
assert.match(artistDetail, /data-artist-album-stage/);
assert.match(artistDetail, /data-artist-record/);
assert.match(artistDetail, /artist-album-controls/);
assert.doesNotMatch(artistDetail, /artist-album-spines/);
assert.match(stylesheet, /\.artist-album-stage \{ max-width:980px; margin-top:24px; overflow:hidden; \}/);
const artistIndexMarkup = archive.archiveArtists();
assert.match(artistIndexMarkup, /class="artist-card-link"/);
assert.doesNotMatch(artistIndexMarkup, />Open artist<\/a>/);
assert.match(stylesheet, /\.artist-card-link \{ position:absolute; z-index:1; inset:0;/);
...
assert.match(shellSource, /data-theme-option="paper"/);
assert.match(shellSource, /data-theme-option="chromatic"/);
assert.match(themeSource, /classList\.toggle\("active", selected\)/);
assert.match(stylesheet, /html\[data-theme="chromatic"\]/);
assert.match(stylesheet, /html\[data-theme="chromatic"\] \.album-card img/);
assert.match(stylesheet, /html\[data-theme="chromatic"\] body\[data-module="archive"\][\s\S]*?linear-gradient/);
assert.match(stylesheet, /html\[data-theme="chromatic"\] body\[data-module="rate"\][\s\S]*?linear-gradient/);
assert.match(homeMarkup, /data-home-landscape-cycle/);
assert.match(homeMarkup, /data-home-landscape-slide/);
assert.match(homeMarkup, /class="album-detail-record featured-landscape-record"/);
assert.match(homeMarkup, /data-home-landscape-next/);
assert.match(homeSource, /const featuredLandscapes =/);
assert.match(homeSource, /showLandscape\(landscapeActive \+ 1\)/);
assert.match(stylesheet, /\.home-landscape-cycle\.landscape-is-changing/);
assert.match(motionSource, /featured-landscape \.ink-draw-wave/);
assert.match(stylesheet, /featuredWaveDrift/);
...
assert.match(stylesheet, /\.album-detail-experience \{ position:relative;/);
assert.match(stylesheet, /\.album-page-wave-background \{ position:fixed/);
