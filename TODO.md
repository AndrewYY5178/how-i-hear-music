...
- [x] Add a per-album `RE-EXTRACT COLOR` action on album detail pages, clear the cached pixel sample, and reapply the current cover tone without changing the stored cover or rating. *(UI 3.11.37; automated checks passed.)*
...
- [x] Render listening scores in a bold serif face across Home, Archive, Rate, Taste and Journal while keeping labels and controls in the existing mono system. *(UI 3.11.46; automated checks passed.)*
- [x] Restore Home album artwork to source colors by removing saturation and sepia filters; retain Cover Flow depth through geometry and record lighting only. *(UI 3.11.47; automated checks passed.)*
- [ ] Add a persistent visual-theme switch with two intentional treatments: `PAPER ARCHIVE` keeps the retro beige / low-saturation paper and sleeve language; `CHROMATIC CONTRAST` uses layered neutral grays for the page surface while preserving original-color album covers and vinyl, with only saved scores using a high-saturation magenta or fluorescent-green accent. Keep layout, data, motion and accessibility behavior shared between themes.
