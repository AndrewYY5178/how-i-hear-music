...
UI 3.7.0 加入了可选的私密云同步：GitHub OAuth 仅用于识别同步账号；上传前，Data Desk 会在浏览器内使用自设同步密码，以 PBKDF2-SHA256（250,000 次）和 AES-GCM 加密完整本地备份。Cloudflare Worker / D1 只保存密文、修订号和最少的账号会话资料，不能读取评分、笔记、Inbox 或 Memory。下载永远先预览数据组与冲突，再由你选择保留本地或采用云端内容并确认；合并后仍提供一次本地完整回滚。同步密码不会上传，也不能由网站找回。
...
个人评分和笔记仍以浏览器本地数据为准。换设备或清理站点数据前，请在 `Import → Data Desk` 导出备份：普通 JSON 便于检查但可直接阅读；密码备份使用 PBKDF2-SHA256（250,000 次）派生密钥并以 AES-GCM 加密。站点不保存也不能找回密码。若要跨设备同步：在 Data Desk 选择“使用 GitHub 登录”，输入至少 10 位同步密码并“上传加密副本”；另一台设备登录同一 GitHub 账号、输入同一密码、选择“下载并预览”，检查冲突策略后再确认合并。
