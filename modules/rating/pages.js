...
export const rateAlbum = (id) => {
  ...
  return `${pageHeader("RATE / ALBUM", safe(album.title), album.artist)}<section class="album-rate-session"><div class="album-wave-heading"><span class="eyebrow mono">BUILD THE LANDSCAPE</span><p>Each point is one track. The curve updates as the album finds its shape.</p></div><div id="album-wave-session">${waveform(draft, { interactive: true })}${summary(draft)}</div><form id="album-rate-form"><div class="album-progress"><span class="mono">TRACK-BY-TRACK</span><span id="album-progress">01 / ${String(draft.length).padStart(2, "0")}</span></div><div class="album-track-controls">${draft.map((track, index) => `<label><span>${String(index + 1).padStart(2, "0")}</span><input data-album-title="${index}" value="${safe(track.title)}"><div><button type="button" data-album-step="-0.1" data-index="${index}">−</button><output>${rating(track.overall)}</output><button type="button" data-album-step="0.1" data-index="${index}">+</button></div></label>`).join("")}</div><label class="album-overall"><span>ALBUM OVERALL</span><input id="album-overall-input" type="number" min="0" max="11" step="0.1" value="${storage.get(draftKey(id) + ":overall", 8.5)}"></label><button class="button primary" type="submit">COMPLETE ALBUM RATING</button></form></section><aside id="album-save-message"></aside>`;
};
