...
export const rateHome = () => {
  const unrated = lifecycleTracks().filter((track) => ["imported", "heard"].includes(track.lifecycleState)); const heard = unrated.filter((track) => track.lifecycleState === "heard").length; const album = allAlbums().find((record) => confirmedAlbumTracks(record).length);
  const albumAction = album ? link(`/rate/album/${album.id || slug(album.artist + "-" + album.title)}`, "RATE AN ALBUM", "button primary") : link("/import/qq-album", "IMPORT AN ALBUM", "button primary");
  return `${pageHeader("RATE", "Begin with one listening decision.", "Choose a shape for one track, or a landscape for an album.")}<div class="rate-choices"><article><span class="mono">01 / TRACK</span><h2>Listening Shape</h2><p>Four dimensions, one personal response.</p>${link(`/rate/track/${trackId(choices[0])}`, "RATE A TRACK", "button primary")}</article><article><span class="mono">02 / ALBUM</span><h2>Listening Landscape</h2><p>${album ? "Build a score curve from a confirmed track order." : "Import a confirmed track order before rating an album."}</p>${albumAction}</article></div><section class="queue-callout"><div><span class="mono">UNRATED QUEUE</span><h2>${heard} heard · ${unrated.length} waiting</h2><p>Imported records stay here until listening becomes a rating.</p></div>${link("/rate/queue", "OPEN QUEUE", "button")}</section><section class="continue-panel"><span class="mono">CONTINUE RATING</span><p>Choose from a confirmed record:</p><div class="inline-links">${choices.slice(0, 8).map((track) => link(`/rate/track/${trackId(track)}`, track.title)).join("")}</div></section>`;
};
...
export const bindRating = (path, navigate) => {
  ...
  if (match) {
    ...
    document.getElementById("track-rate-form").addEventListener("submit", (event) => {
      ...
      const prior = previous ? structuredClone(previous) : null; const priorLifecycle = lifecycleTracks().find((item) => item.id === targetId)?.lifecycleState || "heard";
      saveRating(targetId, saved); updateLifecycle(targetId, "rated"); const journalEntry = appendJournal({ type: "rating", trackId: targetId, title: track.title, artist: track.artist, scores: state, insightTags: reasons, reasons, moment, note, source: track.source || "archive", at: saved.updatedAt });
      const change = delta === null ? "" : `<dl class="rating-change"><div><dt>THEN</dt><dd>${rating(priorOverall)}</dd></div><div><dt>NOW</dt><dd>${rating(nextOverall)}</dd></div><div><dt>CHANGE</dt><dd>${delta > 0 ? "+" : ""}${rating(delta)}</dd></div></dl>`; const returnLink = track.id && !findTrack(targetId) ? link("/rate/queue", "BACK TO QUEUE", "button") : link(`/archive/tracks/${trackId(track)}`, "VIEW TRACK", "button"); const message = document.getElementById("rate-save-message"); message.innerHTML = `<span class='mono'>RATING SAVED</span><p>Saved in this browser${reasons.length ? ` with ${reasons.length} listening reason${reasons.length === 1 ? "" : "s"}` : ""}.</p>${change}<div class="save-actions">${returnLink}<button class="button" type="button" data-undo-rating>UNDO THIS SAVE</button></div>`;
      message.querySelector("[data-undo-rating]")?.addEventListener("click", () => { const ratings = { ...readRatings() }; if (prior) ratings[targetId] = prior; else delete ratings[targetId]; storage.set(visitorKey, ratings); storage.set(journalKey, storage.get(journalKey, []).filter((entry) => entry.id !== journalEntry.id)); if (track.id) updateLifecycle(targetId, prior ? "rated" : priorLifecycle === "rated" ? "heard" : priorLifecycle); message.innerHTML = `<span class="mono">SAVE UNDONE</span><p>The previous rating and lifecycle state were restored. The new Journal entry was removed.</p>`; previous = prior; });
      previous = saved;
    });
  }
  ...
  if (albumMatch) {
    ...
    document.getElementById("album-rate-form").addEventListener("submit", (event) => { event.preventDefault(); const message = document.getElementById("album-save-message"); try { const overall = validScore(document.getElementById("album-overall-input").value); const priorDraft = storage.get(draftKey(id), null); const priorOverall = storage.get(draftKey(id) + ":overall", null); const priorLifecycle = Object.fromEntries(draft.map((track) => [track.trackId, lifecycleTracks().find((item) => item.id === track.trackId)?.lifecycleState || null])); const at = new Date().toISOString(); const { confirmed, previousRatings } = saveAlbumTrackRatings({ album, tracks: draft, at }); storage.set(draftKey(id), confirmed); storage.set(draftKey(id) + ":overall", overall); const journalEntry = appendJournal({ type: "album", albumId: id, title: album.title, artist: album.artist, overall, tracks: confirmed, at }); message.innerHTML = `<span class='mono'>ALBUM COMPLETE</span><p>${safe(album.title)} and ${confirmed.length} Track ratings were saved together.</p><div class="save-actions">${link(`/archive/albums/${id}`, "VIEW ALBUM", "button")}<button class="button" type="button" data-undo-album>UNDO THIS SAVE</button></div>`; message.querySelector("[data-undo-album]")?.addEventListener("click", () => { storage.set(visitorKey, previousRatings); Object.entries(priorLifecycle).forEach(([trackIdValue, state]) => { if (state) updateLifecycle(trackIdValue, state); }); if (priorDraft === null) storage.remove(draftKey(id)); else storage.set(draftKey(id), priorDraft); if (priorOverall === null) storage.remove(draftKey(id) + ":overall"); else storage.set(draftKey(id) + ":overall", priorOverall); storage.set(journalKey, storage.get(journalKey, []).filter((entry) => entry.id !== journalEntry.id)); message.innerHTML = `<span class="mono">SAVE UNDONE</span><p>The prior Track ratings, album draft and Journal were restored.</p>`; }); } catch (error) { message.innerHTML = `<span class="mono">ALBUM NOT SAVED</span><p>${safe(error instanceof Error ? error.message : "The album rating is incomplete.")}</p>`; } });
  }
};
