...
const defaultDraft = (album) => album.tracks?.length ? album.tracks.map((track) => ({ trackId: track.id, title: track.title, overall: readRatings()[track.id]?.scores?.overall ?? 7.5, album: album.title, discNumber: track.discNumber || 1, trackNumber: track.trackNumber })) : ["Track 01", "Track 02", "Track 03", "Track 04", "Track 05", "Track 06"].map((title, index) => ({ title, overall: [7.6, 8.4, 7.9, 9.1, 8.6, 8.2][index], album: album.title }));
...
export const bindRating = (path, navigate) => {
  ...
  if (albumMatch) {
    ...
    document.getElementById("album-rate-form").addEventListener("submit", (event) => { event.preventDefault(); const overall = Number(document.getElementById("album-overall-input").value); const priorDraft = storage.get(draftKey(id), null); const priorOverall = storage.get(draftKey(id) + ":overall", null); storage.set(draftKey(id), draft); storage.set(draftKey(id) + ":overall", overall); const journalEntry = appendJournal({ type: "album", albumId: id, title: album.title, artist: album.artist, overall, tracks: draft, at: new Date().toISOString() }); const message = document.getElementById("album-save-message"); message.innerHTML = `<span class='mono'>ALBUM COMPLETE</span><p>${safe(album.title)} saved with an overall score of ${rating(overall)}.</p><div class="save-actions">${link(`/archive/albums/${id}`, "VIEW ALBUM", "button")}<button class="button" type="button" data-undo-album>UNDO THIS SAVE</button></div>`; message.querySelector("[data-undo-album]")?.addEventListener("click", () => { if (priorDraft === null) storage.remove(draftKey(id)); else storage.set(draftKey(id), priorDraft); if (priorOverall === null) storage.remove(draftKey(id) + ":overall"); else storage.set(draftKey(id) + ":overall", priorOverall); storage.set(journalKey, storage.get(journalKey, []).filter((entry) => entry.id !== journalEntry.id)); message.innerHTML = `<span class="mono">SAVE UNDONE</span><p>The previous album draft was restored and the new Journal entry was removed.</p>`; }); });
  }
};
