...
export const bindRating = (path, navigate) => {
  ...
  if (albumMatch) {
    ...
    document.getElementById("album-rate-form").addEventListener("submit", (event) => { event.preventDefault(); const overall = Number(document.getElementById("album-overall-input").value); storage.set(draftKey(id), draft); storage.set(draftKey(id) + ":overall", overall); appendJournal({ type: "album", albumId: id, title: album.title, artist: album.artist, overall, tracks: draft, at: new Date().toISOString() }); document.getElementById("album-save-message").innerHTML = `<span class='mono'>ALBUM COMPLETE</span><p>${safe(album.title)} saved with an overall score of ${rating(overall)}.</p>${link(`/archive/albums/${id}`, "VIEW ALBUM", "button")}`; });
  }
};
