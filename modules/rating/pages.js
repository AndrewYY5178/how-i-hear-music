import { allTracks, archiveVisibleAlbums, data, findAlbum, findTrack, rating, safe, slug, storage, trackId } from "../music/data.js?v=0.9.111";
...
export const bindRating = (path, navigate) => {
  ...
  if (albumMatch) {
    ...
    const render = (focusIndex = null) => {
      ...
      document.querySelectorAll(".album-track-controls .album-score-input").forEach((input, index) => { input.value = Number.isFinite(Number(draft[index].overall)) ? rating(draft[index].overall) : ""; input.classList.toggle("score-is-rolling", index === focusIndex); input.closest("label")?.classList.toggle("track-is-active", index === focusIndex); });
    ...
    document.getElementById("album-rate-form").addEventListener("change", (event) => { const input = event.target.closest("[data-album-input]"); if (!input) return; const index = Number(input.dataset.index); try { draft[index].overall = validScore(input.value); storage.set(draftKey(id), draft); render(index); } catch { input.value = Number.isFinite(Number(draft[index].overall)) ? rating(draft[index].overall) : ""; input.setCustomValidity("Scores must stay between 0 and 11."); input.reportValidity(); } });
