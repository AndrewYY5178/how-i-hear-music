import { allTracks, archiveVisibleAlbums, data, findAlbum, findTrack, rating, safe, slug, storage, trackId } from "../music/data.js?v=0.9.116";
...
export const rateHome = () => {
  ...
  const scores = next ? (readRatings()[rateId(next)]?.scores || next.scores || {}) : {};
  const landscape = album ? confirmedAlbumTracks(album).map(track => ({ title:track.title, overall:readRatings()[rateId(track)]?.scores?.overall ?? track.scores?.overall ?? null })) : [];
  const previewLandscape = landscape.some(track => track.overall !== null && Number.isFinite(Number(track.overall))) ? landscape : [];
  return `${pageHeader("RATE", "ONE LISTENING DECISION.", "")}<div class="rate-choices sample-rate"><article><span class="mono">01 / TRACK</span><h2>Listening Shape</h2>${radar(scores)}<p>${trackCopy}</p>${trackAction}</article><article><span class="mono">02 / ALBUM</span><h2>Listening Landscape</h2>${waveform(previewLandscape)}<p>${album ? safe(album.title) : "Import a confirmed track order before rating an album."}</p>${albumAction}</article></div><section class="sample-section"><h2>Heard, but not rated.</h2>${unrated.slice(0,3).map(track => `<div class="sample-queue-row">${link('/rate/track/'+encodeURIComponent(rateId(track)),track.title)}<span>${safe(track.artist)}</span></div>`).join('') || '<p>No heard or imported track is waiting for a rating.</p>'}${link('/rate/queue','OPEN FULL QUEUE','button')}</section>`;
};
...
export const bindRating = (path, navigate) => {
  ...
  if (albumMatch) {
    ...
    const render = (focusIndex = null) => {
      ...
      document.querySelectorAll(".album-track-controls .album-score-input").forEach((input, index) => { input.value = hasScore(draft[index].overall) ? rating(draft[index].overall) : ""; input.classList.toggle("score-is-rolling", index === focusIndex); input.closest("label")?.classList.toggle("track-is-active", index === focusIndex); });
      document.getElementById("album-progress").textContent = `${String(draft.filter((track) => hasScore(track.overall)).length).padStart(2, "0")} / ${String(draft.length).padStart(2, "0")}`;
    ...
    document.getElementById("album-rate-form").addEventListener("click", (event) => { const button = event.target.closest("[data-album-step]"); if (!button) return; const index = Number(button.dataset.index); const current = Number(draft[index].overall); draft[index].overall = hasScore(draft[index].overall) ? clampScore(current + Number(button.dataset.albumStep)) : 7.5; storage.set(draftKey(id), draft); render(index); });
    document.getElementById("album-rate-form").addEventListener("change", (event) => { const input = event.target.closest("[data-album-input]"); if (!input) return; const index = Number(input.dataset.index); try { draft[index].overall = validScore(input.value); storage.set(draftKey(id), draft); render(index); } catch { input.value = hasScore(draft[index].overall) ? rating(draft[index].overall) : ""; input.setCustomValidity("Scores must stay between 0 and 11."); input.reportValidity(); } });
    document.getElementById("album-rate-form").addEventListener("input", (event) => { const input = event.target.closest("[data-album-input]"); if (input) input.setCustomValidity(""); });
