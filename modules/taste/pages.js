...
const philosophyItemMarkup = (index, title, copy) => `<details class="essay-item"><summary><span class="essay-item-index">${String(index).padStart(2, "0")}</span><span class="essay-item-title">${safe(title)}</span><span class="essay-item-toggle" aria-hidden="true">+</span></summary><div class="essay-item-body"><p>${safe(copy)}</p></div></details>`;
const philosophyEssayMarkup = () => `<div class="essay-stack">${philosophyItemMarkup(1, "Melody opens the door.", data.profile.firstGateCopy)}${data.profile.listeningOrder.map((item, index) => philosophyItemMarkup(index + 2, item.name, item.note)).join("")}${philosophyItemMarkup(6, "Surprise belongs to the song.", data.profile.surpriseFactor.latePayoffNote)}${philosophyItemMarkup(7, "The human voice stays human.", data.profile.humanVoice.refusal)}</div>`;
