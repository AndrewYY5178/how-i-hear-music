import { allArtists, allTracks, archiveVisibleAlbums, data, findTrack, rating, safe, storage, trackId, visibleJournal, visibleRatings } from "../music/data.js?v=0.9.116";
...
const mergedTasteSections = [
  ["OVERVIEW", "The shape of the archive, its method and its recurring signals.", [["LISTENING DNA", "/taste/dna"], ["LISTENING PORTRAIT", "/taste/portrait"], ["COMPARE WITH ME", "/taste/compare"]]],
  ["JOURNAL", "The time layer: what changed, resurfaced and earned a place in memory.", [["LISTENING JOURNAL", "/taste/journal"], ["ANNUAL INDEX", `/taste/journal/year/${new Date().getFullYear()}`]]],
  ["INSIGHTS", "Interactive analysis of the character and shape of your listening.", [["SONIC MAP", "/taste/sonic-map"], ["LISTENING DNA", "/taste/dna"]]],
];
const philosophyItemMarkup = (index, title, copy) => `<article class="essay-item"><span class="essay-item-index">${String(index).padStart(2, "0")}</span><div class="essay-item-content"><h3 class="essay-item-title">${safe(title)}</h3><p>${safe(copy)}</p></div></article>`;
const philosophyEssayMarkup = () => `<details class="philosophy-disclosure"><summary><span class="mono">LISTENING PRINCIPLES</span><span class="philosophy-disclosure-toggle" aria-hidden="true">+</span></summary><div class="essay-stack">${philosophyItemMarkup(1, "Melody opens the door.", data.profile.firstGateCopy)}${data.profile.listeningOrder.map((item, index) => philosophyItemMarkup(index + 2, item.name, item.note)).join("")}${philosophyItemMarkup(6, "Surprise belongs to the song.", data.profile.surpriseFactor.latePayoffNote)}${philosophyItemMarkup(7, "The human voice stays human.", data.profile.humanVoice.refusal)}</div></details>`;
const philosophyOverviewMarkup = () => `<section class="taste-overview-method"><span class="mono">LISTENING PHILOSOPHY</span><h2>Every element needs a reason.</h2><p>${safe(data.profile.methodCopy)}</p>${philosophyEssayMarkup()}</section>`;
export const tasteHome = () => `${pageHeader("TASTE", "HOW I HEAR MUSIC.", "")}<section class="sample-directory taste-merged-index">${[['Overview','DNA, portrait and comparison.','/taste/dna'],['Journal','What changed over time.','/taste/journal'],['Insights','Explore the archive in space.','/taste/sonic-map']].map(([title,copy,href],i)=>`<a href="${withBase(href)}" data-route><span>0${i+1}</span><h2>${title}</h2><p>${copy}</p><b aria-hidden="true">→</b></a>`).join('')}</section><section class="sample-section"><h2>A few ways in.</h2><nav class="sample-secondary">${mergedTasteSections.flatMap(([, ,items])=>items).filter(([label],i,all)=>all.findIndex(([other])=>other===label)===i).map(([label,href])=>link(href,label,'button')).join('')}</nav></section>${philosophyOverviewMarkup()}`;

// Kept as a renderer for compatibility with imports; the top-level route now redirects to Taste Overview.
export const philosophy = () => `${pageHeader("TASTE / PHILOSOPHY", "Every element needs a reason.", data.profile.methodCopy)}${philosophyEssayMarkup()}`;
...
const profileSummary = () => {
  ...
  return `<section class="taste-profile-summary"><span class="mono">LISTENING PROFILE</span><div class="profile-grid">${traits.map(([label, note]) => `<article><span class="mono">${label}</span><strong>${note}</strong></article>`).join("")}</div><section class="insight-analysis"><span class="mono">WHAT MAKES 9+ TRACKS WORK?</span>${stats.length ? `<div>${stats.map((item) => `<p><strong>${Math.round(item.count / item.total * 100)}%</strong><span>${safe(insightLabel[item.tag])}</span><small>${item.count} OF ${item.total} TAGGED TRACKS</small></p>`).join("")}</div>` : `<h2>Not enough explicit listening reasons yet.</h2>`}</section></section>`;
};

const goodNotMineMarkup = () => {
  ...
  return `<section class="taste-boundary-good"><span class="mono">GOOD ≠ MINE</span><h2>I know it's good. It's just not mine.</h2><p>Technical admiration and personal resonance do not need to agree.</p><div class="good-not-mine">${entries.map((track) => `<article><div>${radar(track.scores, { className: "mini-radar" })}</div><p>${safe(track.artist)}</p><h2>${safe(track.title)}</h2><dl><div><dt>SONG</dt><dd>${rating(track.scores.song)}</dd></div><div><dt>VOCAL</dt><dd>${rating(track.scores.vocal)}</dd></div><div><dt>OVERALL</dt><dd>${rating(track.scores.overall)}</dd></div></dl></article>`).join("")}</div></section>`;
};
...
const boundariesContent = () => {
  ...
  return `${goodNotMineMarkup()}<section class="taste-boundary-patterns"><span class="mono">RECURRING BOUNDARIES</span>${patterns.length ? `<section class="anti-patterns">${patterns.map((pattern) => `<article><span class="mono">PATTERN FROM ${pattern.records.length} TRACKS</span><h2>${safe(pattern.title)}</h2><p>${safe(pattern.copy)}</p><div>${pattern.records.slice(0, 5).map((record) => link(`/archive/tracks/${record.id}`, record.title)).join("")}</div></article>`).join("")}</section>` : `<section class="analysis-empty"><span class="mono">NOT ENOUGH EVIDENCE</span><h2>No boundary repeats across three tracks yet.</h2><p>More explicit ratings may reveal a pattern; the site will not manufacture one.</p></section>`}</section><section class="taste-boundary-blind"><span class="mono">BLIND SPOTS</span>${blind}</section>`;
};
const boundariesMarkup = () => `${pageHeader("TASTE / BOUNDARIES", "What probably won't work for me?", "Patterns describe recurring distance and coverage gaps, never a genre stereotype.")}${boundariesContent()}`;

export const dna = () => {
  const traits = tasteDNA();
  return `${pageHeader("TASTE / DNA", "What repeats beneath the ratings.", "A trait needs at least five supporting tracks. Genre is not used as the primary explanation.")}${profileSummary()}<details class="dna-method"><summary>HOW STRENGTH AND CONFIDENCE WORK <span>+</span></summary><p><b>Strength</b> combines the supporting tracks' average Overall, lift above your archive baseline and score consistency. <b>Confidence</b> increases with evidence count, consistency and recency. Neither number predicts whether a new track will work.</p></details>${traits.length ? `<section class="taste-dna">${traits.map((trait, index) => `<article id="${safe(trait.id)}"><span class="mono">${String(index + 1).padStart(2, "0")}</span><div><h2>${safe(trait.label)}</h2><p>${safe(trait.copy)}</p><small class="mono">${trait.evidenceCount} TRACKS · AVG ${rating(trait.average)} VS ARCHIVE ${rating(trait.baseline)} · ${Math.round(trait.confidence * 100)}% CONFIDENCE</small><details class="dna-evidence"><summary>INSPECT EVIDENCE <span>+</span></summary><div><span class="mono">CONTRIBUTING TRACKS</span>${trait.evidence.map((record) => link(`/archive/tracks/${trackId(record)}`, `${record.title} · ${rating(record.scores?.overall)}`)).join("")}</div>${trait.limitingEvidence.length ? `<div><span class="mono">LIMITING EVIDENCE / BELOW BASELINE</span>${trait.limitingEvidence.map((record) => link(`/archive/tracks/${trackId(record)}`, `${record.title} · ${rating(record.scores?.overall)}`)).join("")}</div>` : `<p>No contributing track falls below the current archive baseline.</p>`}</details></div>${tasteTraitMark(trait)}</article>`).join("")}</section>` : `<section class="analysis-empty"><span class="mono">NOT ENOUGH EVIDENCE</span><h2>No recurring trait reaches five explicit tracks yet.</h2><p>Ratings can contribute to score-based traits. Why This Works, Listening Temperature and history make the model more specific.</p>${link("/rate", "ADD LISTENING EVIDENCE →", "text-link")}</section>`}<section class="taste-dna-boundaries"><span class="mono">BOUNDARIES</span><p>Negative space belongs with the traits it qualifies: what you admire, what resonates and where the archive still has gaps.</p>${boundariesContent()}</section>`;
};

// Retain previous URLs as aliases while presenting one coherent analysis surface.
export const profile = () => dna();
export const goodNotMine = () => boundariesMarkup();
export const antiRecommendation = () => boundariesMarkup();
export const blindSpotPage = () => boundariesMarkup();
