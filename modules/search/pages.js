import { allArtists, allTracks, archiveVisibleAlbums, safe, slug, storage, trackId, visibleJournal } from "../music/data.js?v=0.9.115";
...
export const archiveSearch = () => {
  ...
  archiveVisibleAlbums().filter((album) => includes([album.title, album.artist, album.year], query)).forEach((album) => results.push(row("ALBUM", album.title, album.artist, `/archive/albums/${album.id || slug(album.artist + "-" + album.title)}`, query)));
    ...
    visibleJournal().filter((entry) => includes([entry.title, entry.artist, entry.note, entry.moment?.note], query)).forEach((entry) => results.push(row("JOURNAL", entry.title, entry.note || entry.artist || "Saved listening entry", "/taste/journal", query)));
