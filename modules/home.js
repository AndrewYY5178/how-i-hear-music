import { allAlbums, allTracks, importedAlbums, rating, safe, slug, storage, trackId, visibleJournal, visibleRatings } from "./music/data.js?v=0.9.116";
import { withBase } from "./layout/paths.js";
import { bindCoverTones, fallbackCoverTone } from "./layout/cover-tone.js?ui=3.12.44";
...
export const home = () => {
  ...
  const gate = (path, title, count) => `<a class="newspaper-gate" href="${withBase(path)}" data-route><h2>${title}</h2><span class="mono">${count}</span><span class="newspaper-gate-arrow" aria-hidden="true">↗</span></a>`;
  return `<section class="home-hero newspaper-masthead"><h1 data-i18n-ignore>THE LISTENING<br>ROOM</h1><p>Melody opens the door.<br>Everything else has to earn its place.</p></section><div class="newspaper-lead"><aside class="newspaper-lead-copy"><h2>Listen. Look. Return.</h2><p>A cover, a sequence, a reason to listen again.</p></aside>${listeningSection}</div>${featuredRow}<section class="newspaper-directory" aria-label="Archive categories">${gate('/archive/tracks', 'Tracks', allTracks().length)}${gate('/archive/albums', 'Albums', allAlbums().length)}${gate('/archive/artists', 'Artists', new Set(allTracks().map(track => track.artist)).size)}</section><section class="newspaper-editorial"><article><span class="editorial-label">TASTE / JOURNAL</span><h2>The songs we return to.</h2><p class="journal-dropcap">Some songs arrive slowly. A familiar detail becomes the reason for listening again.</p><a href="${withBase('/taste/journal')}" data-route class="button">READ JOURNAL</a></article><article><span class="editorial-label">ARCHIVE / MY EDITIONS</span><h2>Your own sequence.</h2><p>Choose the songs. Design a cover. Keep a personal edition.</p><a href="${withBase('/archive/compilations')}" data-route class="button primary">MAKE AN EDITION</a></article></section><section class="short-manifesto"><p>Music can be minimal or maximal, familiar or surprising. The only question is whether it stays alive.</p></section>`;
};
