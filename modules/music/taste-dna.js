...
export const blindSpots = ({ records = currentEvidence(), traits = tasteDNA(), albums = [], journal = storage.get("how-i-hear-music:journal:v1", []) } = {}) => {
  ...
  const addConcentrationGap = (field, label) => {
    const known = scored.filter((record) => record[field]); if (known.length < minimumCoverage) return; const groups = new Map(); known.forEach((record) => groups.set(String(record[field]), [...(groups.get(String(record[field])) || []), record])); const [dominant, evidence = []] = [...groups].sort((a, b) => b[1].length - a[1].length)[0] || []; const share = evidence.length / known.length; if (!dominant || share < .7 || groups.size < 2) return;
  ...
  const sonic = readSonic(); const sonicRecords = scored.map((record) => ({ record, sonic: sonic[trackId(record)] })).filter((item) => Number.isFinite(Number(item.sonic?.warmCold)) && Number.isFinite(Number(item.sonic?.denseSparse)));
  if (sonicRecords.length >= 8) { const quadrant = (item) => `${Number(item.sonic.warmCold) < 0 ? "warm" : "cold"} / ${Number(item.sonic.denseSparse) < 0 ? "dense" : "sparse"}`; const counts = new Map(); sonicRecords.forEach((item) => counts.set(quadrant(item), (counts.get(quadrant(item)) || 0) + 1)); const allQuadrants = ["warm / dense", "warm / sparse", "cold / dense", "cold / sparse"]; const [least, count] = allQuadrants.map((name) => [name, counts.get(name) || 0]).sort((a, b) => a[1] - b[1])[0]; const largest = Math.max(...counts.values()); if (count <= 1 && largest >= 4) spots.push({ id: "sonic-quadrant", type: "SONIC COVERAGE GAP", title: `${least} remains nearly empty`, copy: `${count} of ${sonicRecords.length} explicitly placed tracks occupy this Sonic Map quadrant. Character coordinates are not quality predictions.`, affinity: largest / sonicRecords.length, coverageGap: 1 - count / sonicRecords.length, confidence: clamp01(sonicRecords.length / 16), score: (1 - count / sonicRecords.length) * clamp01(sonicRecords.length / 16) * .65, links: [] }); }
