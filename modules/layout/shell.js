import { data, safe } from "../music/data.js?v=0.9.116";
import { accountNickname, saveAccountNickname } from "../music/account.js";
import { beginGithubSync, clearNicknamePrompt, completeEmailSignIn, readSyncStatus, requestEmailCode, signOutSync, syncSession } from "../music/cloud-sync.js?v=0.9.116";
import { withBase } from "./paths.js";
import { currentLanguage, translateText } from "./i18n.js";
import { bindThemeControls, currentTheme } from "./theme.js?v=0.9.116";

const appVersion = "0.9.116";
...
const accountShell = () => {
  ...
  const themeControl = `<div class="account-theme" role="group" aria-label="Visual theme"><span class="mono">THEME</span><div class="account-theme-options"><button class="theme-option${activeTheme === "paper" ? " active" : ""}" type="button" data-theme-option="paper" aria-pressed="${activeTheme === "paper"}">PAPER</button><button class="theme-option${activeTheme === "chromatic" ? " active" : ""}" type="button" data-theme-option="chromatic" aria-pressed="${activeTheme === "chromatic"}">CHROMATIC</button></div></div><button class="paper-sound-toggle mono" type="button" data-paper-sound aria-pressed="true">PAPER SOUND</button>`;
  return `<section class="account-panel" id="account-panel" role="dialog" aria-modal="false" aria-labelledby="account-panel-title" hidden><div class="account-panel-head"><span class="eyebrow mono">ACCOUNT / AUTO SYNC</span></div><h2 class="account-panel-title${signedIn ? "" : " account-signed-out"}" id="account-panel-title" tabindex="-1">${title}</h2>${copy ? `<p>${copy}</p>` : ""}${nicknameForm}${signedIn ? `<div class="account-identity"><span class="mono">${session.user.provider === "email" ? "EMAIL IDENTITY" : "GITHUB IDENTITY"}</span><strong>${session.user.provider === "email" ? safe(session.user.login) : `@${safe(session.user.login)}`}</strong></div>` : ""}<div class="account-actions">${actions}</div>${emailFlow}${themeControl}<p class="account-status mono" id="account-status" aria-live="polite">${signedIn ? "Checking account sync…" : ""}</p><div class="account-version-row"><span class="account-version mono">v${appVersion}</span><button class="account-version-check mono" type="button" data-check-update>CHECK FOR UPDATES</button></div></section>`;
};
