...
import { dataHealth, decryptBackup, encryptedBackupFormat, exportBackup, exportEncryptedBackup, markBackupCreated, recoverySnapshots, restoreBackup, restoreRecoverySnapshot, storageEstimate } from "../music/resilience.js";
...
const restoreFile = async (file, password = "") => {
  if (file.size > 2_000_000) throw new Error("Backup is too large (2 MB maximum).");
  const payload = JSON.parse(await file.text()); const backup = payload.format === encryptedBackupFormat ? await decryptBackup(payload, password) : payload;
...
const bindDataDesk = (navigate) => {
  ...
  document.getElementById("restore-local-data")?.addEventListener("change", async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const restored = await restoreFile(file, document.getElementById("backup-password").value); status.textContent = `${restored} local data groups restored.`; setTimeout(() => navigate("/import/data"), 350); } catch (error) { status.textContent = error instanceof Error ? error.message : "Could not restore this backup."; } });
