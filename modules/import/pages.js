import { archiveVisibleAlbums, canonical, data, safe, slug, storage } from "../music/data.js?v=0.9.116";
...
import { bindCoverTones, fallbackCoverTone, reextractCoverAppearance } from "../layout/cover-tone.js?ui=3.12.44";
...
const importNav = () => secondaryNav([["/import", "Import"], ["/import/inbox", "Inbox"], ["/import/data", "Data Desk"]]) + serviceNotice();
...
export const bindImport = (path, navigate) => {
  if (path === "/import/qq") {
    if (new URLSearchParams(location.search).get('mode') === 'track') {
      document.querySelector('.qq-import-focus').hidden = true;
      document.querySelector('.page-head h1').textContent = translateText('Find a track.');
    }
...
const bindDataDesk = (navigate) => {
  ...
  const coverDesk = document.querySelector(".cover-maintenance-list");
  if (coverDesk) {
    bindCoverTones(coverDesk);
    coverDesk.querySelectorAll(".cover-maintenance-form").forEach((form) => {
      const localFile = form.elements.coverFile; const localPreview = form.querySelector("[data-cover-local-preview]"); const coverId = form.dataset.albumId;
      localFile?.addEventListener("change", () => {
        const file = localFile.files?.[0]; const message = form.querySelector("[data-cover-status]");
        if (!file) { localPreview?.replaceChildren(); return; }
        if (!String(file.type || "").startsWith("image/")) { localFile.value = ""; if (message) message.textContent = "Choose a JPG, PNG, WebP or AVIF image."; return; }
        ...
        if (message) message.textContent = `${file.name} ready. Save cover to keep it on this browser.`;
      });
      form.addEventListener("submit", async (event) => {
        event.preventDefault(); const message = form.querySelector("[data-cover-status]"); const submit = form.querySelector("button[type=submit]"); const file = localFile?.files?.[0]; const value = String(new FormData(form).get("coverUrl") || "").trim();
        if (submit) submit.disabled = true;
        try {
          if (file) {
            const encoded = await encodeLocalCover(file); const local = { ...storage.get(localCoverOverrideKey, {}), [coverId]: encoded }; if (!storage.set(localCoverOverrideKey, local, { recover: false })) throw new Error("The browser could not store this cover. Try a smaller image or clear browser storage.");
            const remote = { ...storage.get(coverOverrideKey, {}) }; delete remote[coverId]; storage.set(coverOverrideKey, remote, { recover: false }); navigate("/import/data"); return;
          }
          ...
          const remote = { ...storage.get(coverOverrideKey, {}), [coverId]: url.href }; if (!storage.set(coverOverrideKey, remote)) throw new Error("The browser could not store this cover reference.");
          const local = { ...storage.get(localCoverOverrideKey, {}) }; delete local[coverId]; storage.set(localCoverOverrideKey, local, { recover: false }); navigate("/import/data");
