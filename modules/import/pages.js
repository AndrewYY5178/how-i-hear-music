...
import { applyDownloadedSync, beginGithubSync, completeGithubSync, downloadEncryptedSync, pushEncryptedSync, readSyncStatus, signOutSync, syncReady, syncSession } from "../music/cloud-sync.js";
...
export const importData = () => {
  ...
  const sync = syncReady() ? `<section class="cloud-sync"><div><span class="eyebrow mono">PRIVATE CLOUD SYNC</span><h2>${session?.user?.login ? `Signed in as ${safe(session.user.login)}.` : "Move only encrypted copies."}</h2><p>GitHub identifies you. Your ratings, notes, Inbox and Memory are encrypted in this browser with the sync password below before any upload. The Worker stores only ciphertext.</p></div><div class="cloud-sync-actions">${session?.token ? `<label class="backup-password"><span class="mono">SYNC PASSWORD</span><input id="sync-password" type="password" minlength="10" autocomplete="current-password" placeholder="The password you will use on every device"></label><div><button class="button primary" id="sync-upload" type="button">UPLOAD ENCRYPTED COPY</button><button class="button" id="sync-download" type="button">DOWNLOAD + PREVIEW</button><button class="button" id="sync-sign-out" type="button">SIGN OUT</button></div>` : `<button class="button primary" id="sync-github-login" type="button">SIGN IN WITH GITHUB</button>`}<div id="cloud-sync-status" aria-live="polite"></div></div></section>` : `<section class="cloud-sync"><span class="eyebrow mono">PRIVATE CLOUD SYNC</span><p>Cloud sync will appear after the hosted service is connected.</p></section>`;
...
const bindDataDesk = (navigate) => {
  const status = document.getElementById("backup-status"); let pendingBackup = null; let pendingCloudBackup = null;
  const syncStatus = document.getElementById("cloud-sync-status");
  const refreshCloudStatus = async () => {
    if (!syncStatus || !syncSession()?.token) return;
    try { const remote = await readSyncStatus(); syncStatus.textContent = remote.updatedAt ? `Encrypted copy available · ${new Date(remote.updatedAt).toLocaleString()}` : "No encrypted copy uploaded yet."; }
    catch (error) { syncStatus.textContent = error instanceof Error ? error.message : "Could not check cloud sync."; }
  };
  completeGithubSync().then((user) => { if (user) navigate("/import/data", { replace: true }); }).catch((error) => { if (syncStatus) syncStatus.textContent = error instanceof Error ? error.message : "Could not complete GitHub sign-in."; });
  ...
  document.getElementById("sync-upload")?.addEventListener("click", async () => { try { const password = document.getElementById("sync-password").value; const result = await pushEncryptedSync(password); document.getElementById("sync-password").value = ""; syncStatus.textContent = `Encrypted copy uploaded · revision ${result.revision}.`; } catch (error) { syncStatus.textContent = error instanceof Error ? error.message : "Could not upload the encrypted copy."; } });
  document.getElementById("sync-download")?.addEventListener("click", async () => { try { const password = document.getElementById("sync-password").value; const result = await downloadEncryptedSync(password); document.getElementById("sync-password").value = ""; if (result.empty) { syncStatus.textContent = "No encrypted copy exists yet."; return; } pendingCloudBackup = result.backup; syncStatus.innerHTML = `<section class="restore-preview"><span class="mono">CLOUD COPY PREVIEW</span><strong>${result.preview.groups} groups · ${result.preview.conflicts} conflicts</strong><p>${result.preview.additions} groups do not conflict with this browser. Nothing has changed yet.</p><label><span class="mono">WHEN THE SAME RECORD EXISTS</span><select id="sync-conflict-policy"><option value="local">KEEP LOCAL VALUES</option><option value="backup">USE CLOUD VALUES</option></select></label><button class="button primary" id="confirm-cloud-merge" type="button">CONFIRM MERGE</button></section>`; syncStatus.querySelector("#confirm-cloud-merge").addEventListener("click", () => { try { const restored = applyDownloadedSync(pendingCloudBackup, syncStatus.querySelector("#sync-conflict-policy").value); pendingCloudBackup = null; syncStatus.innerHTML = `<p>${restored} local data groups merged. A complete rollback is available below.</p>`; setTimeout(() => navigate("/import/data"), 500); } catch (error) { pendingCloudBackup = null; syncStatus.textContent = error instanceof Error ? error.message : "Could not merge the encrypted copy."; } }); } catch (error) { syncStatus.textContent = error instanceof Error ? error.message : "Could not download the encrypted copy."; } });
