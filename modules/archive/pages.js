import { accountSignedIn, allArtists, allTracks, archiveVisibleAlbums, canonical, data, findAlbum, findArtist, findTrack, rating, safe, slug, storage, trackId, visibleJournal, visibleRatings } from "../music/data.js?v=0.9.116";
...
import { archiveSearch } from "../search/pages.js?ui=3.12.44";
import { bindCoverPalette, bindCoverTones, fallbackCoverPalette, fallbackCoverTone, reextractCoverAppearance } from "../layout/cover-tone.js?ui=3.12.44";
...
const albumColorWaves = () => `<div class="album-page-wave-background" aria-hidden="true"><canvas data-album-wave-canvas></canvas></div>`;
...
export const archiveHome = () => {
  const descriptions = ['The details worth keeping.', 'A complete listening sequence.', 'The people behind the record.'];
  const albums = archiveVisibleAlbums().slice(0,4).map(album => { const id = album.id || slug(album.artist + '-' + album.title); const {primary,alternate} = coverSourcesFor(album,id); return `<a class="sample-album" href="${withBase('/archive/albums/'+encodeURIComponent(id))}" data-route><div>${coverMarkup(primary,album.title,false,alternate)}</div><p>${safe(album.artist)}</p><h3>${safe(album.title)}</h3></a>`; }).join('');
  return `${pageHeader('ARCHIVE','THE RECORD INDEX','Tracks, albums and artists that have entered the archive.')}${archiveHomeNav()}${archiveSearch()}<section class="sample-directory">${archiveGates().map(([title,href],i)=>`<a href="${withBase(href)}" data-route><span>0${i+1}</span><h2>${title[0]+title.slice(1).toLowerCase()}</h2><p>${descriptions[i]}</p><b aria-hidden="true">→</b></a>`).join('')}</section><section class="sample-section"><h2>Albums in view.</h2><div class="sample-albums">${albums || '<p>No albums in this archive yet.</p>'}</div></section><section class="newspaper-editorial"><article><h2>Your own editions.</h2>${link('/archive/compilations','OPEN MY EDITIONS','button')}</article><article><h2>Archive coverage</h2>${link('/archive/coverage','REVIEW COVERAGE','button')}</article></section>`;
};
...
export const archiveAlbums = () => `${pageHeader("ARCHIVE / ALBUMS", "Albums in view.", "A cover, artist, and overall response.", link("/archive/compare/albums", "COMPARE ALBUMS", "button"))}${archiveNav()}<div class="album-grid">${archiveVisibleAlbums().sort((left, right) => ratingDescending(left, right, albumOverall, (album) => `${album.artist} ${album.title}`)).map((album) => { const id = album.id || slug(album.artist + "-" + album.title); const { primary: coverUrl, alternate } = coverSourcesFor(album, id); const albumScore = albumOverall(album); const albumTracks = album.tracks?.length ? album.tracks : data.songs.entries.filter((track) => canonicalAlbumMatch(track, album)); const releaseDate = album.releaseDate || album.year || albumTracks.map((track) => track.releaseDate).find(Boolean) || "—"; return `<a class="album-card" href="${withBase(`/archive/albums/${encodeURIComponent(id)}`)}" data-route><div class="album-card-record" data-cover-tone data-cover-source="${safe(coverUrl)}" style="--record-color:${fallbackCoverTone(`${album.artist}-${album.title}`)}"><span class="album-card-disc" aria-hidden="true"></span><div class="album-card-cover">${sleeveDepth}${coverMarkup(coverUrl, album.artist + " — " + album.title + " cover", true, alternate)}</div></div><span class="album-card-terrain mono" aria-label="RATING ${rating(albumScore)} · RELEASE ${safe(releaseDate)}"><span class="album-card-rating">${rating(albumScore)}</span><span class="album-card-release">${safe(releaseDate)}</span></span><p>${safe(album.artist)}</p><h3>${safe(album.title)}</h3></a>`; }).join("")}</div>`;
...
export const archiveAlbumDetail = (id) => {
  ...
  const landscape = confirmed.map((track) => ({ ...track, overall: track.scores?.overall }));
...
export const bindArchive = (path, navigate) => {
  ...
  if (albumMatch) {
    ...
    const colorfield = document.querySelector("[data-album-colorfield]");
    const waveCanvas = bindAlbumWaveCanvas(colorfield);
    bindCoverPalette(record, colorfield).then(() => waveCanvas.refresh());
    record?.addEventListener("him:cover-source-change", (event) => {
      const source = event.detail?.source;
      if (!source) return;
      record.dataset.coverSource = source;
      bindCoverPalette(record, colorfield, { force: true }).then(() => waveCanvas.refresh());
    });
    ...
    reextract?.addEventListener("click", async (event) => { const button = event.currentTarget; const status = document.querySelector("[data-tone-status]"); if (!record) return; button.disabled = true; if (status) status.textContent = translateText("Re-reading cover colors…"); try { const appearance = await reextractCoverAppearance(record, colorfield); waveCanvas.refresh(); if (status) status.textContent = translateText(appearance ? "Cover colors refreshed. The record and wave background now use the new palette." : "Could not re-extract the cover colors."); } catch (error) { if (status) status.textContent = error instanceof Error ? error.message : translateText("Could not re-extract the cover colors."); } finally { button.disabled = false; } });
