...
const orderedTracklist = (tracks) => {
  ...
  return tracks.map((track, index) => {
    ...
    return `${separator}<div><span>${String(track.trackNumber || index + 1).padStart(2, "0")}</span>${link(`/archive/tracks/${trackId(track)}`, track.title)}<b>${rating(track.scores?.overall)}</b></div>`;
  }).join("");
};
...
export const archiveAlbumDetail = (id) => {
  ...
  const albumDraft = history.find((entry) => Array.isArray(entry.tracks))?.tracks || []; const baseTracks = album.tracks || data.songs.entries.filter((track) => canonicalAlbumMatch(track, album)); const savedRatings = storage.get("how-i-hear-music:rating-sessions:v2", {});
