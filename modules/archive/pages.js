import { allAlbums, allArtists, allTracks, canonical, data, findAlbum, findArtist, findTrack, rating, safe, slug, storage, trackId } from "../music/data.js";
...
import { translateText } from "../layout/i18n.js?v=0.9.43";
import { withBase } from "../layout/paths.js";
import { archiveSearch } from "../search/pages.js?ui=3.11.27";
...
export const archiveAlbumDetail = (id) => {
  ...
  const ratingAction = tracks.length ? link(`/rate/album/${id}`, "RATE ALBUM", "button primary") : link("/import/qq-album", "IMPORT TRACK ORDER", "button primary");
  const { primary: coverUrl, alternate } = coverSourcesFor(album, id);
  const localCover = storage.get(localCoverOverrideKey, {})[id] || "";
  const coverReference = `<details><summary>COVER REFERENCE <span>+</span></summary><form class="cover-override-form" data-album-id="${safe(id)}"><label><span class="mono">LOCAL COVER FILE</span><input type="file" name="coverFile" accept="image/jpeg,image/png,image/webp,image/avif"><small>Square images are cropped and compressed in this browser. They are never uploaded.</small></label><div class="cover-local-preview" data-cover-local-preview>${localCover ? `<img src="${safe(localCover)}" alt="Current local cover" referrerpolicy="no-referrer">` : ""}</div><label><span class="mono">REMOTE HTTPS IMAGE URL (OPTIONAL)</span><input type="url" name="coverUrl" value="${safe(storage.get(coverOverrideKey, {})[id] || "")}" placeholder="https://…"></label><div><button class="button" type="submit">SAVE COVER</button><button class="button" type="button" data-clear-cover>USE CANONICAL COVER</button></div><p data-cover-status>${localCover ? "A local cover is active on this browser. Choose another file or use the canonical cover to remove it." : "Use a local file for complex or cross-domain artwork. It stays in this browser and is never uploaded."}</p></form></details>`;
  return `${pageHeader("ALBUM", safe(album.title), safe(album.artist + (album.year ? " · " + album.year : "")), ratingAction)}<section class="album-detail-cover"><div class="album-detail-image">${coverMarkup(coverUrl, album.title + " cover", false, alternate)}</div><div><span class="eyebrow mono">LISTENING LANDSCAPE</span>${waveform(confirmed)}${summary(confirmed)}${narrative ? `<p class="album-narrative">${safe(narrative)}</p>` : ""}</div></section><section><h2>Track list</h2><div class="tracklist">${tracks.length ? orderedTracklist(tracks) : "<p>No confirmed ordered track sequence is available yet.</p>"}</div></section><details><summary>ALBUM CHARACTER <span>+</span></summary><p>${narrative ? safe(narrative) : "A neutral landscape description appears after every track has a confirmed Overall score."}</p></details><details${savedNote ? " open" : ""}><summary>ALBUM NOTES <span>+</span></summary><form class="album-note-form" data-album-id="${safe(id)}"><label><span class="mono">PRIVATE LISTENING NOTE</span><textarea name="note" rows="6" maxlength="2000" placeholder="What stays when the full record ends?">${safe(savedNote?.note || "")}</textarea></label><div><button class="button primary" type="submit">SAVE NOTE</button><button class="button" type="button" data-clear-album-note>CLEAR NOTE</button></div><p data-album-note-status>${savedNote ? `Revised ${safe(new Date(savedNote.revisedAt).toLocaleString())}.` : "Stored locally and included in Data Desk backups."}</p></form></details>${coverReference}<details><summary>RATING HISTORY <span>+</span></summary>${historyMarkup(history)}</details>`;
};
