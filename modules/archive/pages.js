...
export const archiveArtists = () => { const traits = tasteDNA(); return `${pageHeader("ARCHIVE / ARTISTS", "The people at the center.", "Editorial notes first. A signature summarizes recurring geometry without replacing the reason an artist matters.")}${archiveNav()}<div class="artist-grid">${allArtists().map((artist, index) => { const tracks = tracksForArtist(artist.id).map((track) => ({ ...track, scores: resolvedScores(track) })); const artistTraits = traits.filter((trait) => trait.evidence.some((record) => record.artistId === artist.id || record.artist === artist.name)); return `<article><span class="mono">${String(index + 1).padStart(2, "0")}</span>${artistSignature(tracks, artistTraits, `${artist.name} signature`)}<h2>${safe(artist.name)}</h2><p>${safe(artist.role || artist.romanized || "In the archive")}</p>${link(`/archive/artists/${artist.id}`, "Open artist", "text-link")}</article>`; }).join("")}</div>`; };
