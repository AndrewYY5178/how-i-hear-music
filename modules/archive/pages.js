...
export const archiveHome = () => `${pageHeader("ARCHIVE", "Browse the record.", "Tracks, albums and artists that have entered the archive.")}${archiveNav()}<div class="archive-gates">${[["TRACKS", "/archive/tracks", data.songs.entries.length + " recorded tracks"], ["ALBUMS", "/archive/albums", data.profile.albumArchive.length + " albums in view"], ["ARTISTS", "/archive/artists", data.artists.featured.length + " featured artists"]].map(([title, href, note]) => `<article><span class="mono">${title}</span><p>${note}</p>${link(href, "Enter →", "text-link")}</article>`).join("")}</div>`;
