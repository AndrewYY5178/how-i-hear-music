...
export const archiveAlbums = () => `${pageHeader("ARCHIVE / ALBUMS", "Albums in view.", "A cover, an overall response, a compact fingerprint.")}${archiveNav()}<div class="album-grid">${data.profile.albumArchive.map((album) => { const id = slug(album.artist + "-" + album.title); return `<article class="album-card">${coverMarkup(coverFor(album, id), album.artist + " — " + album.title + " cover", true)}<span class="album-fingerprint">▂▄▆█▇▅▆</span><p>${safe(album.artist)}</p><h3>${safe(album.title)}</h3>${link(`/archive/albums/${id}`, "Open album", "card-link")}</article>`; }).join("")}</div>`;
