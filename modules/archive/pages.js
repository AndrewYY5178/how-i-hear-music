...
import { archiveSearch } from "../search/pages.js?ui=3.12.18";
import { bindCoverPalette, bindCoverTones, fallbackCoverPalette, fallbackCoverTone, reextractCoverAppearance } from "../layout/cover-tone.js?ui=3.12.18";
...
const albumColorWaves = () => `<div class="album-color-waves" aria-hidden="true"><svg viewBox="0 0 1200 560" preserveAspectRatio="none"><path class="album-color-wave wave-one" d="M-70 148C108 54 226 250 413 156S726 76 894 178s220 26 376-64v446H-70Z"></path><path class="album-color-wave wave-two" d="M-70 266c160-98 286 68 482-6s305-124 470-16 265 66 388-54v370H-70Z"></path><path class="album-color-wave wave-three" d="M-70 360c186-84 325 48 524-30s315-54 474 48 246 52 342-28v210H-70Z"></path></svg></div>`;
...
export const archiveAlbumDetail = (id) => {
  ...
  return `${pageHeader("ALBUM", safe(album.title), safe(album.artist + (album.year ? " · " + album.year : "")), ratingAction)}<section class="album-detail-colorfield" data-album-colorfield data-cover-album-id="${safe(id)}" style="--album-color-1:${colorOne};--album-color-2:${colorTwo};--album-color-3:${colorThree}">${albumColorWaves()}<section class="album-detail-cover"><div class="album-detail-visual"><div class="album-detail-record" data-album-record data-cover-tone data-cover-album-id="${safe(id)}" data-cover-source="${safe(coverUrl)}" style="--record-color:${fallbackTone};--sleeve-edge-color:${fallbackTone}"><span class="album-detail-disc" aria-hidden="true"></span><div class="album-detail-image" data-cover-tone data-cover-album-id="${safe(id)}" data-cover-source="${safe(coverUrl)}">${coverMarkup(coverUrl, album.title + " cover", false, alternate)}</div></div><div class="album-detail-cover-tools"><button class="button" type="button" data-reextract-tone>RE-EXTRACT COLOR</button><span class="mono" data-tone-status>Theme color is sampled from the current cover.</span><input type="file" name="coverFile" accept="image/jpeg,image/png,image/webp,image/avif" hidden aria-hidden="true"><span class="visually-hidden">Full cover maintenance is available in Data Desk; local cover files are never uploaded.</span></div></div><div><span class="eyebrow mono">LISTENING LANDSCAPE</span>${waveform(landscape)}${summary(landscape)}${narrative ? `<p class="album-narrative">${safe(narrative)}</p>` : ""}</div></section></section><section><h2>Track list</h2><div class="tracklist">${tracks.length ? orderedTracklist(tracks) : "<p>No confirmed ordered track sequence is available yet.</p>"}</div></section><details><summary>ALBUM CHARACTER <span>+</span></summary><p>${narrative ? safe(narrative) : "A neutral landscape description appears after every track has a confirmed Overall score."}</p></details><details${savedNote ? " open" : ""}><summary>ALBUM NOTES <span>+</span></summary><form class="album-note-form" data-album-id="${safe(id)}"><label><span class="mono">PRIVATE LISTENING NOTE</span><textarea name="note" rows="6" maxlength="2000" placeholder="What stays when the full record ends?">${safe(savedNote?.note || "")}</textarea></label><div><button class="button primary" type="submit">SAVE NOTE</button><button class="button" type="button" data-clear-album-note>CLEAR NOTE</button></div><p data-album-note-status>${savedNote ? `Revised ${safe(new Date(savedNote.revisedAt).toLocaleString())}.` : "Stored locally and included in Data Desk backups."}</p></form></details>`;
};
